...
class BootstrapFieldsTest < ActionView::TestCase
  ...
  test "file fields are wrapped correctly with error" do
    ...
    HTML
    assert_equivalent_html expected, bootstrap5_form_for(@user) { |f| f.file_field(:misc) }
  end
  ...
  test "errors are correctly displayed for belongs_to association fields" do
    ...
    HTML
    assert_equivalent_html expected, bootstrap5_form_for(@address, url: users_path) { |f| f.text_field(:user_id) }
  end
  ...
  test "bootstrap5_form_for helper works for associations" do
  ...
  test "bootstrap5_form_for helper works for serialized hash attributes" do
  ...
  test "fields_for correctly passes horizontal style from parent builder" do
    ...
    output = bootstrap5_form_for(@user, layout: :horizontal, label_col: "col-sm-2", control_col: "col-sm-10") do |f|
      f.fields_for :address do |af|
        af.text_field(:street)
      end
    end
  ...
  test "fields_for correctly passes inline style from parent builder" do
    ...
    output = bootstrap5_form_for(@user, layout: :inline) do |f|
      f.fields_for :address do |af|
        af.text_field(:street)
      end
    end
  ...
  test "fields correctly uses options from parent builder" do
    ...
    bootstrap5_form_with(model: @user,
